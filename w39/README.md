...
PS: убиваю оставшиеся монги и меряю память
...
$ sudo free -h
              всего        занято        свободно      общая  буф./врем.   доступно
Память:         31G         16G        4,6G         20M         10G         14G
Подкачка:        8,0G        512K        8,0G
```

В следующих тестах - мерять так же htop-ом.

по монге что мы видим?
во-первых то, что я запускал 12 монг БЕЗ настроек ограничений Тигра по памяти.
В след. раз настрою явно

во-вторых - версия 4 у меня

в-третьих - я не знаю, какой объём коллекции, хотя в статистике шард может это есть в домахе
В след. раз надо выяснить размер коллекции на лям котировок

в-четвёртых - до убийства мастеров всех репликасетов было занято20Г, свободно 1.2Г, после убийста трёх мастеров - занято 18Г, свободно 2.7Г, после погашения всех монг - занято 16Г, свободно 2.7 Г

если арбитров не считать, то три сервера монги жрут 2Г памяти на этой моей коллекции, 6 серверов - 4Г

и это в шардированном виде, то есть данные не дублируются, раскиданы между репликасетами

они дублировались внутри репликасетов, но я поубивал в каждой по одному серверу, оставив по одному

Фикс в память:

репликасет - дублирование данных
шардинг - раскидывание между репликасетами
и это всё у монги в коробке с низким порогом входа
